...
zones   = ["Fort Kochi\n& Mattancherry", "Vyttila\n& Thevara", "Aluva", "Ernakulam\nCity Center", "Kakkanad", "Kalamassery"]
probs   = [0.98, 0.87, 0.82, 0.67, 0.52, 0.38]
levels  = ["RED", "RED", "RED", "ORANGE", "ORANGE", "GREEN"]
...
ax.set_title("LAYER 1 — LSTM Flood Risk Prediction  |  2018 Peak Scenario", fontsize=14, fontweight="bold", color=TEXT, pad=15)
...
nodes = {
    ...
    "SUB_4": ("Fort Kochi\nSub",       "sub",  0.98),
    "SUB_5": ("Aluva\nSub",            "sub",  0.82),
    "PUMP_1": ("Water Pump 1",    "pump", 0.95),
    "PUMP_2": ("Water Pump 2",    "pump", 0.90),
    "PUMP_3": ("Water Pump 3",    "pump", 0.85),
    "PUMP_4": ("Water Pump 4",    "pump", 0.40),
    "PUMP_5": ("Water Pump 5",    "pump", 0.78),
    "HOSP_1": ("General\nHospital",   "hosp", 0.92),
    "HOSP_2": ("Amrita\nHospital",    "hosp", 0.45),
    "HOSP_3": ("Lisie\nHospital",     "hosp", 0.88),
}
edges = [
    ("SUB_1","PUMP_1"),("SUB_1","PUMP_2"),
    ("SUB_3","PUMP_3"),("SUB_4","PUMP_4"),("SUB_5","PUMP_5"),
    ("PUMP_1","HOSP_1"),("PUMP_2","HOSP_2"),("PUMP_3","HOSP_3"),
    ("PUMP_4","HOSP_1"),("PUMP_5","HOSP_3"),
]
for nid in nodes: G.add_node(nid)
for e in edges: G.add_edge(*e)

pos = {
    "SUB_1":  (0,  3), "SUB_2":  (1,  3), "SUB_3":  (2,  3), "SUB_4":  (3,  3), "SUB_5":  (4,  3),
    "PUMP_1": (0,  1.5), "PUMP_2": (1,  1.5), "PUMP_3": (2,  1.5), "PUMP_4": (3,  1.5), "PUMP_5": (4,  1.5),
    "HOSP_1": (1,  0), "HOSP_2": (2.5, 0), "HOSP_3": (3.5, 0),
}

fig, ax = plt.subplots(figsize=(14, 7))
ax.set_title("LAYER 2 — Cascade Dependency Graph  |  RF Edge Failure Probability", fontsize=14, fontweight="bold", color=TEXT, pad=15)
...
ax.text(2, 3.5,  "[SUBSTATIONS]",  ha="center", fontsize=11, fontweight="bold", color=ACCENT)
ax.text(2, 2.1,  "[WATER PUMPS]",   ha="center", fontsize=11, fontweight="bold", color=ACCENT)
ax.text(2, -0.5, "[HOSPITALS]",     ha="center", fontsize=11, fontweight="bold", color=ACCENT)
...
timeline_nodes = ["SUB_4\n(Fort Kochi)", "PUMP_4\n(Water Pump)", "HOSP_1\n(General Hosp.)",
                  "SUB_1\n(Ernakulam)", "PUMP_1\n(Water Pump)", "HOSP_3\n(Lisie Hosp.)"]
fail_hours = [2, 3, 4, 3, 4, 5]
colors_t   = [RED, RED, RED, RED, RED, RED]

fig, ax = plt.subplots(figsize=(12, 6))
ax.set_title("LAYER 2 — Cascade Failure Timeline  |  2018 Peak Scenario", fontsize=14, fontweight="bold", color=TEXT, pad=15)
...
roi_nodes = ["Ernakulam\nMain Sub", "Fort Kochi\nHospital", "Vyttila\nHub Sub", "Aluva\nBridge Node", "Kakkanad\nIT Sub"]
roi_vals  = [1840, 1420, 1210, 980, 750]
lives     = [18400, 14200, 12100, 9800, 7500]
roi_colors= [ACCENT, "#60a5fa", "#93c5fd", "#bfdbfe", "#dbeafe"]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
fig.suptitle("LAYER 3 — ROI Optimizer: Where to Invest to Save the Most Lives", fontsize=14, fontweight="bold", color=TEXT, y=1.01)
...
fig.suptitle("LAYER 1 — LSTM Training Data: 60-Day GPM/WRIS/CWC Proxy  |  2018 Kerala Flood", fontsize=13, fontweight="bold", color=TEXT)
...
axes[0].axvline(PEAK, color=RED, linestyle="--", linewidth=1.5, alpha=0.7, label="2018 Flood Peak (Aug 17)")
...
axes[2].set_title("CWC Reservoir Outflow — Idukki Dam Proxy", fontsize=10, color=MUTED)
axes[2].grid()

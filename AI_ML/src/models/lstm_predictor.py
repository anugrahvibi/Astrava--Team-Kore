...
Trained on synthetic multi-source time-series data mimicking:
  ...
  - CWC reservoir outflow data (cumecs)
"""
...
ZONE_FLOOD_THRESHOLDS = {
    ...
    "ZONE_FORT_KOCHI":   {"orange": 0.6, "red": 0.9,  "base_risk": 0.75},
    "ZONE_VYTTILA":      {"orange": 0.8, "red": 1.1,  "base_risk": 0.60},
    "ZONE_ERNAKULAM":    {"orange": 1.0, "red": 1.4,  "base_risk": 0.50},
    "ZONE_KALAMASSERY":  {"orange": 1.5, "red": 2.0,  "base_risk": 0.30},
    "ZONE_ALUVA":        {"orange": 2.0, "red": 3.0,  "base_risk": 0.45},
    "ZONE_KAKKANAD":     {"orange": 1.2, "red": 1.8,  "base_risk": 0.35},
}


def _generate_synthetic_timeseries(n_days: int = 60, seed: int = 42) -> pd.DataFrame:
    """
    Generate synthetic multi-source time-series data for Kochi, 2018 flood pattern.
...
class LSTMFloodPredictor:
    """
    LSTM-based flood prediction model for Kochi zones.
    ...
    def _compute_lead_time(self, zone_id: str, probability: float) -> int:
        ...
        """
        base_lead = {"ZONE_ALUVA": 18, "ZONE_KALAMASSERY": 14, "ZONE_KAKKANAD": 12,
                     "ZONE_ERNAKULAM": 10, "ZONE_VYTTILA": 8, "ZONE_FORT_KOCHI": 6}
    ...
    def predict_all_zones(self) -> list[dict]:
        """Predict flood risk for all 6 Kochi zones."""
        if not self._trained:
            self.build_and_train()
    ...
    def simulate_scenario(self, scenario: str = "2018_peak") -> list[dict]:
        ...
        scenarios = {
            "2018_peak": {
                "ZONE_FORT_KOCHI":   {"rainfall_mmhr": 4.8, "river_level_m": 1.20, "reservoir_outflow_cumecs": 3200},
                "ZONE_VYTTILA":      {"rainfall_mmhr": 4.2, "river_level_m": 1.05, "reservoir_outflow_cumecs": 3200},
                "ZONE_ERNAKULAM":    {"rainfall_mmhr": 3.9, "river_level_m": 0.95, "reservoir_outflow_cumecs": 3200},
                "ZONE_KALAMASSERY":  {"rainfall_mmhr": 3.1, "river_level_m": 0.75, "reservoir_outflow_cumecs": 3200},
                "ZONE_ALUVA":        {"rainfall_mmhr": 5.2, "river_level_m": 2.80, "reservoir_outflow_cumecs": 3200},
                "ZONE_KAKKANAD":     {"rainfall_mmhr": 3.4, "river_level_m": 0.85, "reservoir_outflow_cumecs": 3200},
            },
            "moderate": {
                "ZONE_FORT_KOCHI":   {"rainfall_mmhr": 2.1, "river_level_m": 0.70, "reservoir_outflow_cumecs": 1800},
                "ZONE_VYTTILA":      {"rainfall_mmhr": 1.8, "river_level_m": 0.65, "reservoir_outflow_cumecs": 1800},
                "ZONE_ERNAKULAM":    {"rainfall_mmhr": 1.6, "river_level_m": 0.55, "reservoir_outflow_cumecs": 1800},
                "ZONE_KALAMASSERY":  {"rainfall_mmhr": 1.4, "river_level_m": 0.40, "reservoir_outflow_cumecs": 1800},
                "ZONE_ALUVA":        {"rainfall_mmhr": 2.5, "river_level_m": 1.20, "reservoir_outflow_cumecs": 1800},
                "ZONE_KAKKANAD":     {"rainfall_mmhr": 1.5, "river_level_m": 0.48, "reservoir_outflow_cumecs": 1800},
            }
        }

      ...
      <div className="w-full lg:w-96 h-auto lg:h-full max-h-[52vh] lg:max-h-none glass-card flex flex-col z-10 shadow-xl shrink-0 rounded-[2.5rem] border border-white/70 bg-white/80 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex flex-col items-end gap-1">
              <select
                ...
                className="text-[9px] font-black bg-blue-50 border border-blue-200 text-blue-600 rounded-lg px-2 py-1 outline-none cursor-pointer hover:bg-blue-100 transition-colors"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[7px] text-gray-400 font-bold uppercase tracking-tighter">Updated {lastUpdated}</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 flex-1 overflow-y-auto space-y-12 custom-scrollbar">
                ...
                {vulnerabilities.tactical_recommendations.slice(0, 3).map((rec, i) => {
                  ...
                  return (
                    <div key={i} className="p-4 bg-orange-50 rounded-3xl border border-orange-100 space-y-2">
                      ...
                      <p className="text-[11px] font-bold text-orange-900 leading-tight">"{strategy}"</p>
                    </div>
                  );
                })}
              </div>
          ...
          {roiRankings && roiRankings.length > 0 && (
            <section className="pt-10 border-t border-gray-100">
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                <ShieldCheck size={12} className="text-emerald-600" /> Strategic Mitigation Hub
              </h3>
              <div className="space-y-3">
                {roiRankings.slice(0, 3).map((roi, i) => (
                  <div key={i} className="p-4 bg-emerald-50 rounded-3xl border border-emerald-100 group hover:shadow-md transition-all">
                    ...
                    <div className="flex items-end justify-between">
                      <div>
                        <div className="text-lg font-black text-emerald-950 leading-none">{roi.lives_saved_per_rupee.toFixed(4)}</div>
                        <div className="text-[8px] font-black text-emerald-600 uppercase tracking-tighter mt-1">Lives Saved Per Rupee</div>
                      </div>
                      <Shield size={16} className="text-emerald-300 opacity-50" />
                    </div>
                  </div>
                ))}
              </div>
